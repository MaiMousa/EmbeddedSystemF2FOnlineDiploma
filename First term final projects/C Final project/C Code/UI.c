...
int main (void)
{
	...
	if (FIFO_Init (&VirtualBuffer, st, Buffer_Length) == FIFO_no_error);	            
	...
	while(1)
	{
		...
		fflush (stdin); fflush(stdout); scanf("%d", &Option);
		printf("\n----------------------------------------------\n");
		switch(Option)
		{
			case 1 :	Add_Student_Manually ();			break;
			case 2 :	Add_Student_File     ();            break;
			case 3 :	Find_by_Roll_Number  ();    		break;
			case 4 :	Find_by_First_Name   ();    		break;
			case 5 :	Find_by_Course       ();    		break;
			case 6 :	Total_Students       ();    		break;
			case 7 :	Delete_Student_by_Roll_Number();	break;
			case 8 :	Update_Student_by_Roll_Number();    break;
			case 9 :	Show_Students        ();		    break;
